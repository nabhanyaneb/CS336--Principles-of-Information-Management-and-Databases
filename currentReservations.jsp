...
<%
			...
			while (rs.next()) { %>
				<tr>    
					<%reservationNumbers.add(rs.getInt("reservationNumber"));%>
...
<br>
	...
	<form method="get" action="cancelReservation.jsp">
	  Choose the reservation number: <select name=reservationNumbers>
 <%for(int i = 0; i < reservationNumbers.size(); i++){
	 %>
	 <option value="<%=reservationNumbers.get(i)%>"><%=reservationNumbers.get(i)%></option>
